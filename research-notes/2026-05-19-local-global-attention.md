...
```text
8bc5e0a experiment: test finer matrix learning rate
```
...
The best result so far is the depth-6 model with quarter-length short windows, the original SSSL local/global layer pattern, and a Muon matrix learning rate of `0.0385`.
...
The matrix LR sweep now points to a narrow optimum. `0.032` and `0.037` were poor, `0.039` had one excellent trace but failed hard on rerun, and `0.038` reproduced with `1.453340` and `1.452655`. The tighter midpoint `0.0385` produced `1.449271` and `1.449275`, with a noisier third run at `1.455500`; it remains the best accepted setting. Neighboring `0.03825` and `0.039` had unreproduced strong traces, and `0.03875` was worse.

Changing nearby optimizer knobs did not help: `WEIGHT_DECAY=0.15`, `WEIGHT_DECAY=0.22`, `WEIGHT_DECAY=0.25`, `EMBEDDING_LR=0.7`, and `UNEMBEDDING_LR=0.003` were all worse. `EMBEDDING_LR=0.65` had one strong trace but failed rerun, so it is not accepted.

Increasing model size to `DEPTH=7` was a clear loss under the fixed-time budget: it raised parameter count to 47.2M, cut trained tokens to 31.5M, and worsened BPB to `1.617837`.

## Next Overnight Queue

1. Try `EMBEDDING_LR = 0.625` with `MATRIX_LR = 0.0385`.
2. Try `WEIGHT_DECAY = 0.18` with `MATRIX_LR = 0.0385`.
3. Try `UNEMBEDDING_LR = 0.005` with `MATRIX_LR = 0.0385`.
4. Try `SCALAR_LR = 0.4` with `MATRIX_LR = 0.0385`.
5. Try a third confirmation run only for any new candidate that beats `1.449271` by less than `0.002` BPB.
