...
The matrix LR sweep now points to a narrow optimum. `0.032` and `0.037` were poor, `0.039` had one excellent trace but failed hard on rerun, and `0.038` reproduced with `1.453340` and `1.452655`. The tighter midpoint `0.0385` reproduced almost exactly (`1.449271`, `1.449275`) and is the current best. Changing nearby optimizer knobs did not help: `WEIGHT_DECAY=0.25`, `EMBEDDING_LR=0.7`, and `UNEMBEDDING_LR=0.003` were all worse.

## Next Overnight Queue

1. Try `MATRIX_LR = 0.03825` as a tighter lower midpoint.
2. Try `MATRIX_LR = 0.03875` as a tighter upper midpoint, with rerun if promising.
3. Try a third confirmation run of `MATRIX_LR = 0.0385` if later improvements are below `0.002` BPB.
4. Try `WEIGHT_DECAY = 0.22` with `MATRIX_LR = 0.0385`.
5. Try `EMBEDDING_LR = 0.65` with `MATRIX_LR = 0.0385`.
